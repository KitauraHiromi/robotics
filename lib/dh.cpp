...
tform dh::h(vd param, double theta){
    tform ret;
    ret(0, 0) = cos(theta);
    ret(0, 1) = -sin(theta)*cos(param[1]);
    ret(0, 2) = sin(theta)*sin(param[1]);
    ret(0, 3) = param[0]*cos(theta);
    ret(1, 0) = sin(theta);
    ret(1, 1) = cos(theta)*cos(param[1]);
    ret(1, 2) = -cos(theta)*sin(param[1]);
...
dh make_puma(double l1, double l2, double l3){
    ...
    puma.param[2][0] = 0; puma.param[2][1] = RAD(90); puma.param[2][2] = 0;
